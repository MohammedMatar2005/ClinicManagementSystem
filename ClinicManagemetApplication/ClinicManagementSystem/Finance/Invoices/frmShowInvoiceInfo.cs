    ...
    public partial class frmShowInvoiceInfo : Form
    {
        ...
        private async Task _FillData()
        {
            try
            {
                ...
                var patientName = $"{invoice.Visit?.Appointment?.Patient?.Person?.FirstName} " +
                    $"{invoice.Visit?.Appointment?.Patient?.Person?.SecondName} " +
                    $"{invoice.Visit?.Appointment?.Patient?.Person?.LastName}";
