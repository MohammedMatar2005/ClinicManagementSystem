...
public class clsInvoiceStatusData
{
    ...
    public static bool DeActivateAnInvoiceStatus(int InvoiceStatusId)
    {
        int rowsAffected = 0;
        using (SqlConnection connection = new SqlConnection(DataAccessSettings.ConnectionString))
        {
            using (SqlCommand command = new SqlCommand("Sp_InvoiceStatuses_DeActivateStatus", connection))
            {
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@StatusId", InvoiceStatusId);
